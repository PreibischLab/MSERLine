...
public class LinefinderHough extends BenchmarkAlgorithm implements OutputAlgorithm<ArrayList<CommonOutput>> {
	...
	public LinefinderHough (final RandomAccessibleInterval<FloatType> source, 
		...
		// Create the Bit image for distance transform and seed image for watershedding
					...
					RandomAccessibleInterval<BitType> bitimg = new ArrayImgFactory<BitType>().create(source, new BitType());
					GetLocalmaxmin.ThresholdingBit(source, bitimg, ThresholdValue);
	...
	public boolean process() {
		...
		for (int label = 1; label < Maxlabel - 1; label++) {
			 ...
			 * 
			 */
			CommonOutput currentOutput = new CommonOutput(framenumber, label, slopeandintercept, roiimg, ActualRoiimg, interval);
