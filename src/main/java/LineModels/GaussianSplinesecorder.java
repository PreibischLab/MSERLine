...
public class GaussianSplinesecorder implements MTFitFunction {
	...
	public double grad(double[] x, double[] a, double[] b, int k) {
		...
		if (k < ndims) {
			...
			for (int i = 0; i < ndims; ++i) {
				...
				mplus2bx = 1 + b[ndims] + 2 * a[2 * ndims + 1] * a[0];
				if (i == 0)
				...
				start += 2 * b[i] * (x[i] - a[i]) * a[2 * ndims + 2] * dxvector[i] * Estartds(x, a, b);

			}

			for (int i = ndims; i < 2 * ndims; ++i) {
				int dim = i - ndims;
				mplus2bx = 1 + b[ndims] + 2 * a[2 * ndims + 1] * a[ndims];
				if (dim == 0)
				...
				end += 2 * b[dim] * (x[dim] - a[i]) * a[2 * ndims + 2] * dxvector[dim] * Eendds(x, a, b);
	...
	private static final double Esumds(final double[] x, final double[] a, final double[] b) {
		...
		double curvature = a[2 * ndims + 1];
		double ds = Math.abs(a[2 * ndims]);
		double newslope = (maxVal[1] - minVal[1]) / (maxVal[0] - minVal[0]) - curvature * (maxVal[0] + minVal[0]);
	...
	private static final double EsumC(final double[] x, final double[] a, final double[] b) {
		...
		double dxderiv = -ds * 2 * minVal[0] * (slope + 2 * curvature * minVal[0])
				/ Math.pow(1 + Math.pow((slope + 2 * curvature * minVal[0]), 2), 3 / 2);
		double[] dxvectorderiv = { dxderiv, dxderiv * (slope + 2 * curvature * minVal[0]) };

		double dxderivend = -ds * 2 * maxVal[0] * (slope + 2 * curvature * maxVal[0])
				/ Math.pow(1 + Math.pow((slope + 2 * curvature * maxVal[0]), 2), 3 / 2);
